LOOP:   LODD PasCnt:	; Loads the number of passes left								; for (i = PasCnt; i > 0; i--)
		JZER DONE:		; If no more passes, end program
		SUBD c1:		; Decrements number of passes
		STOD PasCnt:	; Store decremented number of passes
P1:		LODD daddr:		; Loads address of current argument into accumulator			; arg = data[PasCnt - i]
...
P2:		PUSH			; Pushes result onto stack
...
FIB:	LODL 1			; Loads argument from stack into accumulator
...
CLEAR:	INSP 1			; Removes latest argument from stack
...
FIBZER:	LODD c0:		; Stores 0 into accumulator
		RETN			; Returns back to loop or recursion
FIBONE:	LODD c1:		; Stores 1 into accumulator
		RETN			; Returns back to loop or recursion
DONE:	HALT			; Ends program
	
.LOC 	100			; locate data beginning at 100
data:  		3			; array of args for fib function
     		9
     		18
     		23
     		25
results:	0		; array of result locs for fib returns
     		0
     		0
     		0 
     		0
